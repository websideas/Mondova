...
add_action('after_switch_theme', 'delphinus_woocommerce_set_option', 1);
...
if (!function_exists('cruxstore_cart_link')) {
    function cruxstore_cart_link($class = 'cart-contents', $text = null)
    {
        ...
        printf(
            '<a href="%s" class="%s" title="%s"><span>%s<span class="amount">%s</span> - <span class="subtotal">%s</span></span></a>',
...
function cruxstore_template_loop_product_actions()
{
    ...
    if(!cruxstore_option('catalog_mode', 0)){
        ...
        if (defined('YITH_WOOCOMPARE')) {
            printf(
                '<div data-toggle="tooltip" title="%s" >%s</div>',
...
add_action('woocommerce_after_add_to_cart_button', 'cruxstore_template_product_actions');
