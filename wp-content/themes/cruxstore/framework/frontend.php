...
function cruxstore_setting_script() {
    ...
    if($mega_border_color = cruxstore_option('mega_border_color', '#ebebeb')){
        $mega_border_arr = array(
            '#nav #main-navigation > li .cruxstore-megamenu-wrapper.megamenu-layout-table > ul > li > ul > li',
            ...
            '#nav #main-navigation > li .cruxstore-megamenu-wrapper.megamenu-layout-table > ul > li'
        );
                ...
                if($data){
                    switch($service) {
                        case "facebook":
                            ...
                            if(is_array($data)){
                                $count = (count($data)) ? $data[0]->total_count : 0;
                            }else{
            ...
            foreach($share_arr as $key => $val){
                if($val){
                    ...
                    if($key == 'facebook'){
                        ...
                        /*if($count){
                            $html .= '<span class="count">'.cruxstore_entry_share_count('facebook').'</span>';
                        }*/
                        $html .= '</a></li>';
                    }elseif($key == 'twitter'){
                        // Twitter
    ...
    function cruxstore_related_article($post_id = null, $type = 'categories'){
        ...
        if($query->have_posts()){ ?>
            <div id="related-article">
                <h3 class="post-single-heading"><?php esc_html_e('Related Article', 'cruxstore'); ?></h3>
